...
class BattleInProgressResult {
	...
	// example, suppose both players committed hashes of their battle plans in block n. They then
	// have until block (n + Constants.MAX_BLOCKS_AFTER_HASHES_COMMITTED) to reveal their battle
	// plans. If one player does so and the other doesn't, then the latter player forfeits and the
	// height of the forfeit is (n + Constants.MAX_BLOCKS_AFTER_HASHES_COMMITTED). There is no
	// message at this height, though, so we don't have an OnChain<BattlePlan> from which to read
	// the block ID. In such cases, we simply set finishBlockId to zero. Worst-case scenario, we
	// roll back an extra block or two while resolving a fork.
