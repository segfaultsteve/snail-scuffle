...
public class AccountsTest {
	...
	public void updateAccount() throws AccountsException {
		...
		StatChangesFromBattle changes = changesFromBattle(account1, account2, Constants.INITIAL_SYNC_HEIGHT + 1, 1);
	...
	public void orderCachedBlocksByHeight() throws AccountsException {
		...
		for (int i = 0; i < 5; i++) {
			StatChangesFromBattle changes = changesFromBattle(account1, account2, ++currentHeight, i);
			accounts.update(Arrays.asList(changes));
		}
	...
	public void rollBackToCurrentHeight() throws AccountsException {
		...
		for (int i = 0; i < 5; i++) {
			StatChangesFromBattle changes = changesFromBattle(account1, account2, ++currentHeight, i + 1);
			accounts.update(Arrays.asList(changes));
	...
	private static StatChangesFromBattle changesFromBattle(Account winner, Account loser, int height, long blockId) {
		...
		return new StatChangesFromBattle(
			height,
			blockId,
			new PlayerResult(winner.numericId(), winner.rating, newRatings.winner, winner.streak, newWinnerStreak),
			new PlayerResult(loser.numericId(), loser.rating, newRatings.loser, loser.streak, newLoserStreak)
		);
	}
